...
sed -i "s/For help on using BigBlueButton see these/Para obtener ayuda sobre el uso de BigBlueButton, puede consultar/g"  /usr/share/bbb-web/WEB-INF/classes/bigbluebutton.propertie>
...
sudo ln -s /datadrive/bigbluebutton /var/bigbluebutton
...
## Crontab -e
#@reboot bbb-conf --restart >> /var/log/bbbrestart.log
#@reboot   mkdir -p /mnt/scalelite-recordings
#@reboot mount -a
#0 7 * * * /bin/bash /home/azureuser/scalelite_batch_import.sh
#0 9 * * * /bin/bash /ansible/recoveryRecordingsJobV2.sh >> /ansible/logs/recoveryRercordingsJobV2.log
